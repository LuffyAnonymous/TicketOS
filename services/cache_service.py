...
def launch_auto_cache_if_needed(rows):
    ...
    for row in rows:
        ...
        if not source or not order_id: continue
        existing = get_cached_order_details(source, order_id)
        if should_refresh_cached_details(existing): pending.append(row)
    if not pending: return
    with context.AUTO_CACHE_MANAGER["lock"]:
        if context.AUTO_CACHE_MANAGER["running"]: return
        context.AUTO_CACHE_MANAGER["running"] = True
    def runner():
        try:
            context.state.log(f"Auto-cache: sync started for {len(pending)} order(s)")
            auto_cache_order_details(pending)
        except Exception as e: context.state.log(f"Auto-cache error: {e}")
        finally:
            with context.AUTO_CACHE_MANAGER["lock"]: context.AUTO_CACHE_MANAGER["running"] = False
    threading.Thread(target=runner, daemon=True).start()

def fetch_and_cache_order_details_for_row(row):
    source = clean_text(row.get("source", ""))
    order_id = clean_text(row.get("id", ""))
    if not source or not order_id: return False
    existing = get_cached_order_details(source, order_id)
    if existing and not should_refresh_cached_details(existing): return False
    try:
        if source == "LiveTicketGroup":
            ...
            if adapter:
                ...
                context.state.log(f"Auto-cache: loaded {source} #{order_id}")
                return True
        return False
...
def auto_cache_order_details(rows, max_threads=AUTO_CACHE_THREADS):
    ...
    for row in rows:
        ...
        if not source or not order_id: continue
        existing = get_cached_order_details(source, order_id)
        if should_refresh_cached_details(existing): pending_rows.append(row)
    ...
    context.state.log("Auto-cache cycle complete")
