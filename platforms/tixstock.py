...
class TixstockAdapter(OrderPlatformAdapter):
    def __init__(self, config):
        super().__init__("Tixstock", config)

    def fetch_orders(self):
        username = self.config.get("username", "").strip()
        password = self.config.get("password", "").strip()
        login_url = self.config.get("login_url", "https://www.tixstock.com/login")
        orders_url = self.config.get("orders_url", "https://www.tixstock.com/orders")

        if not username or not password:
            raise RuntimeError("Tixstock: username/password not configured")

        context.state.log("Tixstock: starting browser for check")
        with sync_playwright() as p:
            ...
            try:
                ...
                browser.close()
                raise RuntimeError(f"Tixstock scraping failed: {str(e)}")
